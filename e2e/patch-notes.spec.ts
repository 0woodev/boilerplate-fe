...
// (playwright.config starts vite with VITE_USE_MSW=true). Exercises the full
// user path: login → view changelog → create → edit.
test('patch notes: login, view, create, edit', async ({ page }) => {
  // --- Login (MSW mocks /auth/login) ---
  await page.goto('/login')
  await page.locator('#username').fill('demo')
  await page.locator('#password').fill('demo')
  await page.getByRole('button', { name: '로그인' }).click()

  // --- Land on home, navigate to Patch Notes ---
  ...
  await createDialog.locator('#create-body').fill('**Playwright** 로 작성한 노트\n\n- 항목 1\n- 항목 2')
